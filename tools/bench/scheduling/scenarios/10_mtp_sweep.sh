...
# Patches only [SCHED_MODEL] in models-lab.ini, restarts lab, restores keys afterwards.
set -euo pipefail
...
orig_type="$(get_lab_ini_section_key "$section" "spec-type")"
orig_nmax="$(get_lab_ini_section_key "$section" "spec-draft-n-max")"
...
restore_mtp() {
  if [[ -n "$orig_type" ]]; then
    patch_lab_ini_section "$section" "spec-type" "$orig_type"
  fi
  if [[ -n "$orig_nmax" ]]; then
    patch_lab_ini_section "$section" "spec-draft-n-max" "$orig_nmax"
  fi
}
...
for n in "${MTP_VALUES[@]}"; do
  ...
  if [[ "$restart" != "1" ]]; then
    ...
    if [[ "$n" == "off" ]]; then
      ...
      patch_lab_ini_section "$section" "spec-type" "none"
    else
      [[ "$n" =~ ^[0-9]+$ ]] || die "invalid MTP depth: $n (want off|1|2|…)"
      log "MTP draft-mtp n-max=$n — patch [${section}]"
      patch_lab_ini_section "$section" "spec-type" "draft-mtp"
      patch_lab_ini_section "$section" "spec-draft-n-max" "$n"
    fi
    restart_lab_patched
  fi
