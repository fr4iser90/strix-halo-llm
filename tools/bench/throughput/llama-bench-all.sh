#!/usr/bin/env bash
# llama-bench: benchmark chat GGUFs on Vulkan, ROCm, and/or CPU.
# Stops routers first — bench and serve share the GPU.
set -euo pipefail
...
[[ "$NO_RESTORE" -eq 1 ]] && export BENCH_NO_RESTORE=1
  ...
  if [[ "$want_vk" -eq 1 || "$want_rocm" -eq 1 ]]; then
    ...
    if command -v docker >/dev/null 2>&1; then
      ...
      bench_restore_on_exit() {
        [[ "${BENCH_STOPPED_ROUTERS:-0}" -eq 1 ]] || return 0
        [[ "${BENCH_NO_RESTORE:-0}" == "1" ]] && return 0
        [[ -f "$PROJECT_ROOT/tools/bench/lib/routers.sh" ]] || return 0
        # shellcheck source=../lib/routers.sh
        source "$PROJECT_ROOT/tools/bench/lib/routers.sh"
        bench_restore_after_throughput
        BENCH_STOPPED_ROUTERS=0
      }
      trap bench_restore_on_exit EXIT
    fi
  fi
fi
